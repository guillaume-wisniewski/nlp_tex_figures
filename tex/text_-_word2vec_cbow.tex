...
\begin{document}
    ...
    \begin{tikzpicture}
        ...
        \node[draw=airforceblue, minimum width=.5cm, minimum height=\onehotsize{}cm] (w1) at (0cm,0cm) {};
        \node[left=of w1] {ailes};
        \pgfmathsetmacro{\pos}{55} % \pos en pourcentage
        \draw[airforceblue, fill=airforceblue!50] (w1.south west) ++(0, \pos/100*\onehotsize) rectangle ++(0.5, 0.1);


        \node[draw=airforceblue, minimum width=.5cm, minimum height=\onehotsize{}cm, below=of w1] (w2) {};
        \node[left=of w2] {de};
        \pgfmathsetmacro{\pos}{70} % \pos en pourcentage
        \draw[airforceblue, fill=airforceblue!50] (w2.south west) ++(0, \pos/100*\onehotsize) rectangle ++(0.5, 0.1);

        \node[draw=white, minimum width=.5cm, minimum height=\onehotsize{}cm, below=of w2] (w3) {};
        \node[left=of w3] {\texttt{[MASK]}};

        \node[draw=airforceblue, minimum width=.5cm, minimum height=\onehotsize{}cm, below=of w3] (w4) {};
        \node[left=of w4] {l'};
        \pgfmathsetmacro{\pos}{10} % \pos en pourcentage
        \draw[airforceblue, fill=airforceblue!50] (w4.south west) ++(0, \pos/100*\onehotsize) rectangle ++(0.5, 0.1);

        \node[draw=airforceblue, minimum width=.5cm, minimum height=\onehotsize{}cm, below=of w4] (w5) {};
        \node[left=of w5] {empêche};
        \pgfmathsetmacro{\pos}{30} % \pos en pourcentage
        \draw[airforceblue, fill=airforceblue!50] (w5.south west) ++(0, \pos/100*\onehotsize) rectangle ++(0.5, 0.1);

        \node[draw=airforceblue, fill=airforceblue!50, right=of w3, xshift=4cm, minimum height=3cm, minimum width=.5cm] (hidden) {};

        \node[draw=airforceblue, fill=airforceblue!50, right=of hidden, xshift=4cm, minimum height=6cm, minimum width=.5cm] (score) {};
